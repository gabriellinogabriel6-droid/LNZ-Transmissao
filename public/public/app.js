...
async function startSharing() {
  ...
  try {
    const stream = await navigator.mediaDevices.getDisplayMedia({
      ...
      // Pede ao navegador para NÃO oferecer o áudio geral do computador.
      systemAudio: 'exclude',
      // Evita capturar a própria página por engano quando o navegador suporta.
      selfBrowserSurface: 'exclude',
      surfaceSwitching: 'include'
    });

    // Segurança extra: só permitimos áudio quando a fonte escolhida é uma ABA do navegador.
    // Se a pessoa escolher Tela inteira ou Janela, removemos qualquer áudio recebido.
    const displaySurface = stream.getVideoTracks()[0]?.getSettings?.().displaySurface || '';
    if (displaySurface !== 'browser' && stream.getAudioTracks().length) {
      stream.getAudioTracks().forEach((track) => {
        track.stop();
        stream.removeTrack(track);
      });
      if (state.shareAudio) {
        showToast('Áudio bloqueado: use uma aba do navegador para transmitir som sem captar o Discord.');
      }
    }
...
function updateAudioControl() {
  ...
  button.disabled = state.isSharing;
}

$('audioControl').addEventListener('click', () => {
  if (state.isSharing) return showToast('Pare a transmissão antes de mudar o áudio.');
  state.shareAudio = !state.shareAudio;
  updateAudioControl();
  showToast(state.shareAudio ? 'Áudio da aba ativado. O áudio do sistema/Discord continuará bloqueado.' : 'Áudio desligado.');
});
